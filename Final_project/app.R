...
ui <- fluidPage(
    ...
    tags$div(
        ...
        titlePanel("COVID-19 Data Visualization Dashboard")
    ),
    ...
    fluidRow(
        column(12, 
               tags$div(
                   ...
                   h4("Welcome to the COVID-19 Data Visualization Dashboard!"),
                   p("This dashboard provides visualizations of COVID-19 data trends, allowing you to explore and analyze key metrics."),
        ...
        navbarMenu("Case Number",
                   tabPanel("Plot: Case Number",
                            sidebarPanel(
                                ...
                                # New dropdown input
                                selectInput("year_1_1", "Select A Year:",
                                            choices = unique(df$Year)),
                                # New dropdown input
                                selectInput("month_1_1", "Select A Month:",
                                ...
                                # New checkbox input
                                checkboxInput("show_labels_1_1", "Show labels on bars", value = FALSE),
                                # Horizon Line
                                hr(),
                                # Download Button
                                downloadButton(
                            ...
                            mainPanel(
                                ...
                                br(),
                                hr(),
                                p("To utilize this app, manipulate the widgets on the side to customize the visualization according to your requirements! Additionally, you can download a high-quality image of your plot using the download button. The data dictionary is as follows:"),
                                tags$ul(
                                    ...
                                    tags$li(tags$b("Reported Cases"), " - the number of cases reported for the selected COVID scenario, state, and demographic group in the chosen year and month"),
                                    tags$li(tags$b("Reported Hospitalizations"), " - the number of hospitalizations reporting the case number for the selected COVID scenario, state, and demographic group in the chosen year and month"),
                                )
                            )
                   ),
        ...
        navbarMenu("Case Number by Age Band",
                   tabPanel("Plot: Case Number by Age Band",
                            sidebarPanel(
                                ...
                                # New dropdown input
                                selectInput("year_2_1", "Select A Year:",
                                            choices = unique(df1$Year)),
                                # New dropdown input
                                selectInput("month_2_1", "Select A Month:",
                                            choices = month.abb),  # Use month.abb for month dropdown
                                # New dropdown input
                                ...
                                # New checkbox input
                                checkboxInput("show_labels_2_1", "Show labels on bars", value = FALSE),
                                # Horizon Line
                                hr(),
                                # Download Button
                                downloadButton(
                   ...
                   tabPanel("Plot: Inpatient Bed Number",
                            ...
                            mainPanel(
                                ...
                                tags$ul(
                                    ...
                                    tags$li(tags$b("Reported Hospitalizations"), " - the number of hospitalizations reporting the bed number for the selected state in the chosen year and month"),
                                )
                            )
                   ),
                   ...
                   tabPanel("Plot: Distribution of Death Cases by State",
                            ...
                            mainPanel(
                                ...
                                tags$ul(
                                    tags$li(tags$b("Year"), " - the year when the data were collected"),
                                    tags$li(tags$b("Month"), " - the month when the data were collected"),
                                    tags$li(tags$b("Reported Cases"), " - the number of patients with suspected or confirmed COVID-19 who died in the hospital, ED, or overflow location for the selected state in the chosen year and month"),
                                    tags$li(tags$b("Reported Hospitalizations"), " - the number of hospitalizations reporting the patient number with suspected or confirmed COVID-19 who died in the hospital, ED, or overflow location for the selected state in the chosen year and month"),
                                )
                            )
                   ),
...
server <- function(input, output, session) {
    ...
    observe({
        updateSelectInput(session, "month_1_1", choices = get_available_months(input$year_1_1))
    ...
    filtered_data <- reactive({
        df %>%
            filter(state == input$state_1_1 & 
                       Year == input$year_1_1 & 
                       Month == match(input$month_1_1, month.abb) &
    ...
    plot1_1 <- reactive({
        filtered_data_plot1 <- if(input$covid_1_1 != "All Types") {
            filtered_data() %>% filter(covid == input$covid_1_1)
        } else {
            filtered_data()
        }
        
        p <- ggplot(filtered_data_plot1, aes(x = covid, y = value, fill = coverage)) +
        ...
        if (all(filtered_data_plot1$value == 0)) {
            p <- p + ylim(0, 100) +
                labs(x = "Case Type",
                     y = "Number",
                     caption = "Data Source: U.S. Department of Health & Human Services") +
                theme_minimal() + 
                scale_fill_discrete(name = "", labels = c("Reported Hospitalizations", "Reported Cases")) +
                theme(text=element_text(size=14, family = "Times New Roman")) 
        } else {
            p <- p + ylim(0, 1.2*max(filtered_data_plot1$value)) +
                labs(x = "Case Type",
                     y = "Number",
                     caption = "Data Source: U.S. Department of Health & Human Services") +
                theme_minimal() +
    ...
    output$download_epicurve_1 <- downloadHandler(
        ...
        filename = function() {
            stringr::str_glue("Plot_COVID_{input$group_1_1}_{input$state_1_1}_{input$month_1_1}_{input$year_1_1}.png")
        },
    ...
    # Update month dropdown based on selected year
    observe({
        updateSelectInput(session, "month_2_1", choices = get_available_months(input$year_2_1))
    ...
    # code to render plot 2
    filtered_data2 <- reactive({
        df1 %>%
            filter(state == input$state_2_1 & 
                       Year == input$year_2_1 & 
                       Month == match(input$month_2_1, month.abb) &
                       group == input$group_2_1)
    })
    ...
    plot2_1 <- reactive({
        filtered_data_plot2 <- if(input$covid_2_1 != "All Types") {
            filtered_data2() %>% filter(covid == input$covid_2_1, Age == input$age_2_1)
        } else {
            filtered_data2() %>% filter(Age == input$age_2_1)
        }
        
        p <- ggplot(filtered_data_plot2, aes(x = covid, y = value, fill = coverage)) +
            geom_bar(stat = 'identity', position = 'dodge') 
        ...
        # Dynamically set y-axis limits
        if (all(filtered_data_plot2$value == 0)) {
            p <- p + ylim(0, 100) +
                theme_minimal() + 
                labs(x = "Case Type",
                     y = "Number",
            ...
            p <- p + ylim(0, 1.2*max(filtered_data_plot2$value)) +
                theme_minimal() +
                labs(x = "Case Type",
                     y = "Number",
                     caption = "Data Source: U.S. Department of Health & Human Services") + 
    ...
    output$download_epicurve_2 <- downloadHandler(
        ...
        filename = function() {
            stringr::str_glue("Plot_COVID_{input$age_2_1}_{input$group_2_1}_{input$state_2_1}_{input$month_2_1}_{input$year_2_1}.png")
        },
    ...
    filtered_data21 <- reactive({
        df1 %>%
            filter(state %in% input$state_2_2 & 
                       Year == input$year_2_2 & 
                       Month %in% match(input$month_2_2, month.abb) &
                       Age %in% input$age_2_2)
    })
    
    # Update reactiveValues when data changes
    observe({
        data_values1$filtered_data_table2 <- if(input$covid_2_2 != "All Types") {
            filtered_data21() %>% filter(covid == input$covid_2_2)
        } else {
            filtered_data21()
        }
    })
    ...
    observe({
        data_values$filtered_data_table1 <- if (input$group_2_2 != "All Groups") {
            filtered_data21() %>% filter(group == input$group_2_2)
        } else {
            filtered_data21()
        }
    })
